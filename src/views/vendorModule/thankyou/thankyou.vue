...
onMounted(() => {
    ...
    (async function(){
        ...
        const tip       = route.query.tip
            ...
            console.log('response________thankyou----', response.data)
            if (response.data.status == 'VERIFIED'){
            ven_dta.value = response.data.ven_p_type
            console.log('ven_p_type',ven_dta.value)
            console.log('ven_p_type------:', ven_dta.value, 'Type:', typeof ven_dta.value);
        ...
        {vid.value   =   response.data.vid; ven_dta.value = response.data.ven_dta;ven_dta.value = response.data.ven_p_type; fullpageloader.value = false;   }
                ...
                <v-card elevation="10" class="border ctm-thankyou">
                    <v-card-item>
                        ...
                        <div class="pt-14 pb-10">
                            ...
                            <!-- Conditionally render based on v_role value -->
                             
                            <div v-if="ven_dta === 'close'" class="d-sm-flex align-center justify-center gap-3 mt-5 mb-5">
                                <v-btn color="primary" class="rounded-sm" variant="outlined" @click="questiontwo = true">Skip & go to dashboard</v-btn>
                                <v-btn color="primary" class="mt-4 mt-sm-0 rounded-sm" to="/profile-settings">Complete your profile</v-btn>
                            </div>

                            <div v-else class="d-sm-flex align-center justify-center gap-3 mt-5 mb-5">
                                <v-btn color="primary" class="rounded-sm" variant="outlined" @click="questionone = true">Skip & go to dashboard</v-btn>
                                <v-btn color="primary" class="mt-4 mt-sm-0 rounded-sm" to="/profile-settings">Complete your profile</v-btn>
                            </div>
