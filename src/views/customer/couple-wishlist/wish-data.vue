...
async function SavedForLater_listing() {
 
 const formData = new FormData();
//  formData.append("category_id", categoryId);
 const response = await axios.post(`/home/fetch_SavedForLater`, formData, config);
 console.log(response.data.final_data,"response.data.dataresponse.data.data")
 if (response.data.status == 'Done') {

     console.log('this is done with data ----',response.data.final_data)
     bookingData.value = response.data.final_data.map((elem: any) => ({
        id: elem.package?.id ?? '',
         serviceservice: elem.package.category?.name ?? '',
         vendorserviceimg: elem.package.package_logo === '/media/default.jpg' ? serviceimg1 : BASE_URL + (elem.package.package_logo ?? '/media/default.jpg'),
         serviceprovidername: elem.package.name ?? '',
         vendorserviceprice: elem.package.price ? `$${elem.package.price}` : '',
         a_payemnt: elem.package.advance_payment ? `${elem.advance_payment}` : '',
         vendorname: `${elem.package.user?.first_name ?? ''} ${elem.package.user?.last_name ?? ''}`,
        //  business_name: elem.package.vendor?.business_name ?? '',
        business_name: elem.package?.vendor ? elem.package.vendor.business_name ?? '' : '',

         deposit: elem.package.deposit_amount ?? '',
         first_price: elem.package.first_price ?? '',
         full_price: elem.package.full_price ?? '',
         
         role: elem.package.vendor ? 'vendor' : 'Admin',
         link: `/vendor-single/${elem.package.id ?? ''}`,
         iratings:elem.package.iratings,
         // link: elem.vendor ? `/vendor-single/${elem.id ?? ''}` : `/vendor-single_admin/${elem.id ?? ''}`,
         // location: elem.Cities ?? '',
         location: elem.package.cities.map((a:any)=>(a.city)).join(', '),
     }));

    //  searchbudgets.value = response.data.package.budget
     console.log("finally______________", bookingData.value)
 fullpageloader.value=false;
 }
}
SavedForLater_listing()
...
async function remove_package(itemId) {
    try {
                            ...
                            <v-card elevation="10" class="vendor-card-hover h-100">
                                <v-card-item class="pa-4">
                                    <div class="position-relative d-flex overflow-hidden">
                                        ...
                                        <div >
                                        <v-col cols="6">
                                    <div color="transparent" class="ctm_save"
                                        @click="remove_package(item.id)">
