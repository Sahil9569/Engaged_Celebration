...
async function email_listing() {
    try {
        ...
        if (response.data.status === 'SENT') {
            ...
            email_data.value = response.data.cust_emails.map(item => {
                // Calculate content length based on subject length
                ...
                // Truncate receivers if length > 18
                const allReceivers = [
                ...
                return {
                    id: item.id,
                    read_unread: item.couple_read_unread,
                    mail_link: "/single-mail/" + item.id,
...
async function deleteSelectedEmails() {
    ...
    try {
        ...
        // formData.append("customer_delete_email", 'customer_delete_email');
        const response = await axios.post('/home/delete_emails', { customer_delete_email: 'customer_delete_email', email_ids: selectedEmailIds });
            ...
            <v-card elevation="0" rounded="md" class="bg-grey100 border-0 h-100">
                <v-card-item class="">
                    <v-row class="justify-content-between align-center pt-3">
                        <v-col cols="6" md="6"> <v-card-title class="text-h4 ">Email Inbox
                            </v-card-title>
                        </v-col>
                                        ...
                                        <div class="d-flex justify-space-between align-center">
                                            <div class="d-flex align-center">
                                                <div class=" d-flex align-center" style="width: 200px;">
                                                    ...
                                                    <h6 v-if="item.read_unread == false"
                                                        class="text-subtitle-1 font-weight-bold text-dark">To: {{
                                                        item.receivers }}
                                                    </h6>
                                                    <h6 v-else class="text-subtitle-1 font-weight-normal text-dark">To:
                                                        {{ item.receivers }}
                                                    </h6>
                                                </div>
                                                <router-link class="text-decoration-none" v-if="item.id"
                                                    :to="{ name: 'Single Mail', params: { id: item.id } }">
                                                    <div
                                                        class="position-relative vendorsc-section d-flex overflow-hidden">
                                                        <div class="">
                                                            <h6 v-if="item.read_unread == false"
                                                                class="text-subtitle-1 font-weight-bold text-dark"> {{
                                                                item.subject }} - <span
                                                                    class="text-subtitle-1 font-weight-regular text-Darklight">{{
                                                                    item.content }}</span>
                                                            </h6>

                                                            <h6 v-else
                                                                class="text-subtitle-1 font-weight-normal text-dark"> {{
                                                                item.subject }} - <span
                                                                    class="text-subtitle-1 font-weight-regular text-Darklight">{{
                                                                    item.content }}</span>
                                                            </h6>
