...
async function package_listing() {
  try {
    ...
    console.log("response_______listing_", response)
...
const filteredList = computed(() => {
    ...
    function checkbudget(input:any, range:any){
        ...
        return (input >= parseInt(range.replace(/\$/g, "").split(" - ")[0]) && input <= parseInt(range.replace(/\$/g, "").split(" - ")[1])) ? true : false
    }
...
async function pack_status_toggle(pack=null,pack_status=null){
    ...
    const response = await axios.post('home/package_listing', formdata)
        ...
        <v-col cols="12">
            <v-card elevation="0" rounded="md" class="bg-grey100 border-0 h-100">
                <v-card-item class="pa-sm-6 pa-3">
                    <v-row class="justify-content-between pt-3 align-center">
                        ...
                        <v-col cols="12" md="9" class="d-flex gap-2 align-center flex-wrap">
                            <v-row>
                                ...
                                <v-col cols="12" md="3" class="">
                                    <v-select v-model="searchcat" :items="searchcats" item-text="text" item-value="value" label="Select Categories" single-line variant="outlined" hide-details return-object  density="compact"></v-select>
                                </v-col>
                                <v-col cols="12" md="3" class="">
                                    <v-btn color="secondary" variant="flat" hide-details class="w-100"
                                        :to="'/add-package'">
                            ...
                            <v-card elevation="10" class="vendor-card-hover h-100">
                                <v-card-item class="pa-4">
                                    ...
                                    <div class="position-relative d-flex overflow-hidden">
                                        ...
                                        <router-link class="w-100 d-flex img_div" :to="'/edit-package/' + item.id" v-if="item.package_logo !== '/media/default.jpg'">
                                            <img :src="BASE_URL + item.package_logo" class="rounded vendormainimg">
                                        </router-link>

                                        <router-link class="w-100 d-flex img_div" :to="'/edit-package/' + item.id" v-else >
                                            <img :src="serviceimg1" class="rounded vendormainimg">
                                        </router-link>
                                    ...
                                    <div >
                                        <router-link class="text-decoration-none text-black" :to="'/edit-package/' + item.id">
