...
const meadmin = (localStorage.getItem('role') == 'admin') ? true : false
...
const mypack            = ref({});
const bdetails          = ref({brandValue : '', wedstates : '', businesslocation : '', EventCovered : '', bdescription : '', blocation : '', bcats: ''});
const weblinks          = ref([])
const pdcounts          = ref([])
const vendoractive      = ref(true);
const vendorstatusdialog = ref(false);
...
const canchat = (localStorage.getItem('role') == 'vendor') || JSON.parse(String(localStorage.getItem('user'))).id == vid ? false : true
async function basic_detailfirst() {
  fullpageloader.value = true
  const config = { headers: { "Content-Type": "application/json" }, withCredentials: true }
  const response = await axios.post(`/home/vendor_onboard_data`, {'vid':vid}, config);
  ...
  if (response.data.status == 'SAVED'){
    ...
    bdetails.value.blocation    =   ven.location_link
    bdetails.value.wedstates    =   ven.weddings_states.map( (a:any) => (a.state) ).join(', ')
    bdetails.value.bcats        =   ven.category.map( (a:any) => (a.name) ).join(', ')
    weblinks.value              =   ven.weblinks.map( (a:any) => (a.url) )
    // cats.value                  =   ven.category.map( (a:any) => ({text:a.name, value:a.id}) )
    vendoractive.value          =   response.data.vendoractive


    portfolio.value             =   response.data.serialized_albums.map((a:any) => ({ portfolioname: a.name, imgasd: (BASE_URL + a.album_bckpic), images: a.image_urls.map((c)=>({image:`${BASE_URL}/media/${c}`})), imgs: a.image_urls.map((c)=>(`${BASE_URL}/media/${c}`)) }))
...
async function updatevstatus(){
    fullpageloader.value = true
    const response = await axios.post('home/updatevstatus', {"vid": vid})
    if (response.data.status == "SUCCESS"){
        showSuccessMessage.value    = true;
        myerrors.value              = response.data.msg;
        basic_detailfirst()
    }
    vendorstatusdialog.value = false
}

const updateStatus = () => {
  const currentIndex = statusitem.value.indexOf(vendoractive.value);
  const nextIndex = (currentIndex + 1) % statusitem.value.length;
  vendoractive.value = statusitem.value[nextIndex];
  // Here you can add logic to send update request to the server if needed
  vendorstatusdialog.value = false; // Close the menu
}; 
...
async function package_listing() {
  try {
    const config = { headers: { "Content-Type": "application/json" }, withCredentials: true }
    const response = await axios.post(`/home/package_listing`, {'vpid':vid}, config);
...
<div class="loader" v-if="fullpageloader"><span class="loader-17"> </span><div class="loaderinner"><img src="@/assets/images/wedding/mainloader.png" /></div></div>
    ...
    <v-row class="mb-4">
        <v-col cols="12" md="4" lg="4" xl="3">
            <!-- <v-card elevation="10" class="mb-6 userbackPaper border">
                <v-card-item class="publiceProfiledesignafter">
                    <v-row class="ps-4 pb-5 pt-2">
                        <v-col cols="12" class="position-relative">
                            <img src="/src/assets/images/backgrounds/label.png" width="150">
                            <h4 class="text-h6 text-white publiceProfiledesign">Public Profile</h4>
                        </v-col>
                        
                        <v-col cols="12" md="6" class="d-flex flex-column justify-end align-end">
                            
                            <div class="d-flex align-end justify-start  py-1 gap-2">
                                <v-btn variant="flat" color="success" class="rounded-sm" size="small" to="/profile-settings/basicdetails" v-if="canchat"> <MessageIcon stroke-width="1.5" size="15" class="mr-1" />Chat</v-btn>
                            </div>
                        </v-col>
                    </v-row>
                </v-card-item>
                    ...
                    <v-row class="mt-1 ma-3 py-0">
                        ...
                        <v-col cols="12" sm="4" class="d-flex align-start justify-center justify-sm-end order-sm-third">
                            <div class="d-flex align-center justify-end  py-1 gap-2">
                                ...
                                <v-menu v-model="vendorstatusdialog" :close-on-content-click="false" anchor="top">
                                    ...
                                    <v-card min-width="300">
                                        ...
                                        <v-list>
                                            <v-list-item >
                                                <div class="my-4 pt-3">
                                                    <v-autocomplete v-model="vendoractive" :items="statusitem" density="compact" label="Change Status" color="primary" variant="outlined" hide-details></v-autocomplete>
                                                </div>
                                            </v-list-item>
                                        </v-list>
                                        ...
                                        <v-btn text @click="vendorstatusdialog = false" flat class="rounded-sm bg-error" size="small"> Close </v-btn>
                                        <v-btn color="success" text @click="vendorstatusdialog = false" flat class="rounded-sm bg-success" size="small"> Update </v-btn>
                                        </v-card-actions>
                                    </v-card>
                                </v-menu>
                            </div>
            ...
            <v-card elevation="10" class="overflow-hidden mb-5 border">
                <v-card-item class="pb-0 border-bottom userbackPaper position-relative ">
                    <v-chip rounded="lg" class="font-weight-semibold mb-3 bg-success" size="small" label v-if="vendoractive">Current status: Active</v-chip>
                    <v-chip rounded="lg" class="font-weight-semibold mb-3 bg-error" size="small" label v-else>Current status: Suspended</v-chip>
                    <div class="tagprofile">
                        Public Profile
                    </div>
                    <div class="align-center d-flex flex-column justify-center text-center position-relative">
                        <v-avatar  size="100" class="ml-n2 avtar-border mb-3">
                            <img :src="ppic" alt="usericon" height="100"/>
                        </v-avatar>
                        <div class="mb-8">
                            <div class="">
                                <!-- <div class="d-flex align-center mb-3 gap-2 justify-center">
                                    <v-chip
                                        rounded="lg"
                                        :class="[vendoractive === 'Active' ? 'font-weight-semibold success' : 'font-weight-semibold error']"
                                        size="small"
                                        :color="[vendoractive === 'Active' ? 'success' : 'error']"
                                        label>{{ vendoractive }}</v-chip>
                                ...
                                <h5 class="text-h4 mb-2 text-13 text-capitalize">{{f_name}} {{l_name}}</h5>
                                <div class="mb-2">
                            ...
                            <div class="">
                                <div class="text-subtitle-2 d-flex align-center font-weight-semibold justify-center"><UserExclamationIcon  size="15" stroke-width="2" class="me-1"/>{{mypack.ptype}} Vendor</div>
                                <!-- <div class="text-subtitle-2 d-flex align-center font-weight-semibold"><LocationIcon  size="15" stroke-width="2" class="me-1"/>{{city}}, {{zipmodal}}</div> -->
                            </div>
                            <!-- <v-col cols="12" sm="4" class="d-flex align-start justify-center justify-sm-end order-sm-third">
                                <div class="d-flex align-center justify-end  py-1 gap-2">
                                    <v-menu v-model="vendorstatusdialog" :close-on-content-click="false" anchor="top">
                                        ...
                                        <v-card min-width="300">
                                            ...
                                            <v-btn text @click="vendorstatusdialog = false" flat class="rounded-sm bg-error" size="small"> Close </v-btn>
                                            <v-btn color="success" text @click="updatevstatus()" flat class="rounded-sm bg-success" size="small"> Update </v-btn>
                                            </v-card-actions>
                                        </v-card>
                                    </v-menu>
                                </div>
                            </v-col> -->
                            <div class="d-flex align-end justify-center  pt-3 gap-2" v-if="canchat">
                                <v-btn variant="flat" color="success" class="rounded-sm" size="small" :to="msglink" > <MessageIcon stroke-width="1.5" size="15" class="" />Chat</v-btn>
                            </div>
                            <div class="d-flex align-end justify-center  pt-3 gap-2" v-if="meadmin">
                ...
                <v-card-item class="pb-0 border-bottom pa-sm-6 pa-3">
                    <div class="mb-8">
                    <h4 class="text-h6 text-Darklight mb-4">Personal Information</h4>
                    <div>
                        <p class="text-subtitle-1 d-flex align-start font-weight-medium mb-5">
                            <span class="text-Darklight d-flex align-center me-1  ">
                                <UserIcon size="18" stroke-width="2" class="me-1"/> Name :  
                            </span>
                            <span class="text-capitalize">{{f_name}} {{l_name}}</span>
                        </p>
                        <p class="text-subtitle-1 d-flex align-start font-weight-medium mb-5">
                            <span class="text-Darklight d-flex align-center me-1  ">
                ...
                <v-card-item class="pb-14 pa-sm-6 pa-3">
                <div class="mb-2">
                    <h4 class="text-h6 text-Darklight mb-4">Contacts</h4>
                    <div>
                        <p class="text-subtitle-1 d-flex align-start font-weight-medium mb-5 d-flex flex-column">
                            <span class="text-Darklight d-flex align-center mb-2 ">
                                <MailIcon size="18" stroke-width="2" class="me-1"/> Email :  
                            </span>
                            <span class="ps-6">{{u_email}}</span>
                        </p>
                        <p class="text-subtitle-1 d-flex align-start font-weight-medium  d-flex flex-column">
                            <span class="text-Darklight d-flex align-center mb-2 ">
                                <PhoneIcon size="18" stroke-width="2" class="me-1"/> Phone Number :  
                            </span>
                            <span class="ps-6">{{ccode}} {{u_phone}}</span>
                        </p>
                    </div>
                </div>
                </v-card-item>
            </v-card> 
            <!-- <v-card elevation="10" class="bg-dark">
                <v-card-item class="py-12">
                    <div class="text-center">
                        <div class="v-card-title text-h1 font-weight-semibold pa-0 text-center text-primary">4.0
                        </div>
                        <v-rating half-increments disabled v-model="rating" density="compact" color="" size="small" class="text-primary"></v-rating>
                        <router-link to="/" class="textDark text-center">
                            <div class="v-card-title text-h5 pa-0 d-flex justify-center align-center gap-2 text-primary">Total Reviews
                                <ArrowUpRightIcon stroke-width="2   " size="15" />
                            </div>
                        </router-link>
                        <p class="text-primary mb-0 text-subtitle-1">Review From Couples</p>
                    </div>
                </v-card-item>
            </v-card> -->
        </v-col>
        <v-col cols="12" md="8" lg="8" xl="9">
            <v-row class="">
                <v-col cols="12" md="6" class="px-2 py-2">
                    <v-card elevation="10" class="bg-grey200 h-100">
                        <v-card-item class="px-5 py-5">
                            <div class="v-card-title text-h4 font-weight-semibold pa-0 ">${{pdcounts[0]}}
                            </div>
                            <div  class="textDark">
                ...
                <v-col cols="12" md="6" class="px-2 py-2">
                    <v-card elevation="10" class="bg-grey200 h-100">
                        <v-card-item class="px-5 py-5">
                            <div class="v-card-title text-h4 font-weight-semibold pa-0 ">{{pdcounts[1]}}
                            </div>
                            <router-link to="/admin-reports" class="textDark">
                                <div class="v-card-title text-subtitle-1 font-weight-semibold pa-0 d-flex align-center gap-2">
                                        Total Bookings
                                        <ArrowUpRightIcon stroke-width="2   " size="15" />
                                </div>
                ...
                <v-col cols="12" md="6" class="px-2 py-2">
                    <v-card elevation="10" class="bg-grey200 h-100">
                        <v-card-item class="px-5 py-5">
                            <div class="v-card-title text-h4 font-weight-semibold pa-0 d-flex align-center gap-2">{{pdcounts[2]}}
                            </div>
                            <router-link to="/admin-reports" class="textDark">
                ...
                <v-col cols="12" md="6" class="px-2 py-2">
                    <v-card elevation="10" class="bg-grey200 h-100">
                        <v-card-item class="px-5 py-5">
                            <div class="v-card-title text-h4 font-weight-semibold pa-0 d-flex align-center gap-2">{{pdcounts[3]}}
                            </div>
                            <router-link to="/admin-reports" class="textDark">
            ...
            <v-card elevation="10" class="overflow-hidden  mt-6">
                <img :src="bpic" alt="profile" class="w-100 profileBannerImg" />
                <v-card-item class="pt-3 pa-sm-6 pa-3">
                    <div class="mb-8">
                        ...
                        <div>
                            <p class="text-subtitle-1 d-flex align-md-center align-start font-weight-medium mb-4">
                                <span class="text-Darklight d-flex align-center me-1 text-no-wrap">
   ...
   :  
                            ...
                            <p class="text-subtitle-1 d-flex align-md-center align-start font-weight-medium mb-4">
                                <span class="text-Darklight d-flex align-center me-1 text-no-wrap">
                                    <BuildingIcon size="18" stroke-width="2" class="me-1"/> Offered States :  
                                </span>
                                <span class="">{{ bdetails.wedstates }}</span>
                            </p>
                            <p class="text-subtitle-1 d-flex align-md-center align-start font-weight-medium mb-4">
                                <span class="text-Darklight d-flex align-center me-1 text-no-wrap">
                            ...
                            <p class="text-subtitle-1 d-flex align-md-center align-start font-weight-medium mb-4">
                                ...
                                <p class="text-subtitle-1 font-weight-medium mb-4">
                                    <span class="text-Darklight d-flex align-center me-1 mb-2 ">
                                        <FileDescriptionIcon size="18" stroke-width="2" class="me-1"/> Business Description :  
                                    </span>
                                    <div class="setHeightDec scroll-style border pa-3 rounded" style="line-height:1.6em;font-size:13px;" v-html="bdetails.bdescription">
                                    </div>
                                </p>
                        </div>
                    </div>
        ...
        <v-col cols="12" md="12">
            <v-card elevation="0" class="border">
                ...
                <v-row>
                    ...
                    <!-- <v-col cols="12" sm="4" class="d-flex align-center justify-center justify-sm-end order-sm-third">
                        <div class="d-flex align-center justify-end  py-1 gap-3">
                            <v-btn variant="flat" color="primary" class="rounded-sm" size="small" to="/profile-settings/portfolios"> View Portfollios</v-btn>
                        </div>
                    </v-col> -->
                </v-row>
                ...
                <div class="position-relative">
                    ...
                    <carousel ref="carouselfirstRef" :settings="portoslideSettings" :breakpoints="portoslideBreakpoints"
                                ...
                                <v-card>
                                    ...
                                    <v-card-item>
                                        <v-row class="pa-2">
                                            ...
                                            <!-- <template v-if="card.image.url.startsWith('data:image')">
                                                <img :src="card.image" alt="gallery" width=""
                                                            class="albumsingleimg rounded" />
                                            </template>
                                            <template v-else-if="card.image.url.startsWith('data:video')">
                                                <video :src="card.image.url" controls></video>
                                            </template> -->
        ...
        <v-col cols="12" md="12">
            <v-card elevation="0" class="border">
                ...
                <v-row>
                    ...
                    <!-- <v-col cols="12" sm="4" class="d-flex align-center justify-center justify-sm-end order-sm-third">
                        <div class="d-flex align-center justify-end  py-1 gap-3">
                            <v-btn variant="flat" color="primary" class="rounded-sm" size="small" to="/packages"> View Packages</v-btn>
                        </div>
                    </v-col> -->
                </v-row>
                <div class="position-relative">
                            ...
                            <v-card elevation="10" class="vendor-card-hover h-100">
                                <v-card-item class="pa-4">
                                    ...
                                    <div >
                                        ...
                                        <h6 class="text-body-2 text-13 text-start"><span class="font-weight-semibold">Deposit
    ...
    <v-row v-if="weblinks.length">
        <v-col cols="12" md="12">
            <v-card elevation="0" class="border" >
                <v-card-item>
                    <v-row>
                        <v-col cols="12" sm="8">
                            <h4 class="text-h6 text-Darklight ">Website Links</h4>
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col cols="auto" v-for="link in weblinks"> 
                            <v-card elevation="10" class="border bg-translate bg-grey100">
                                <v-card-item class="">
                                    <a :href="link" target="_blank" class="gap-3 d-flex align-center text-primary-custom justify-space-between text-decoration-none">
                                        <div class="widthSet1"><p class="text-wrap  text-dark text-body-2  font-weight-medium" :width="200">{{ link }}</p></div>
                                        <div class="widthSet d-flex justify-end align-center text-dark"> <ArrowUpRightIcon size="20"/> </div>
                                    </a>
                                </v-card-item>
                            </v-card>
                        </v-col>
...
.userbackPaper{
    background: url(@/assets/images/website/banner4.jpg);
    background-size: cover;
    background-repeat: no-repeat;
    background-position: center;
}
...
// .publiceProfiledesign[data-v-1b086085] {
//     position: absolute;
//     top: 41%;
//     left: 20px;
//     transform: translateY(-50%);
//     z-index: 999;
// }
// .publiceProfiledesignafter[data-v-1b086085]:after {
//     content: "";
//     width: 227px;
//     height: 40px;
//     position: absolute;
//     top: 16px;
//     left: 0;
//     background-image: url(/src/assets/images/backgrounds/label.png);
